...
test("keeps the latest thinking inline even when an abort appends an empty message", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      ...assistantEntry("a1", "u1", "partial answer"),
      message: {
        role: "assistant",
        provider: "test",
        model: "test-model",
        content: [
          { type: "thinking", thinking: "in-flight reasoning" },
  ...
  // 中止追加的空消息之后的思考消息仍保留内联，避免中止后重新加载
  assert.equal(deferred.messages[1].content[0].thinking, "in-flight reasoning");
  assert.equal(deferred.messages[1].content[0].deferred, undefined);
});

test("defers historical thinking but keeps the latest assistant message inline", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      ...assistantEntry("a1", "u1", "old answer"),
    ...
    {
      ...assistantEntry("a2", "u2", "latest answer"),
      message: {
        role: "assistant",
        provider: "test",
        model: "test-model",
  ...
  const deferred = buildSessionContext(entries, undefined, { deferThinking: true });
  // 历史 assistant 的思考剥离为 deferred
  assert.deepEqual(deferred.messages[1].content[0], {
    type: "thinking",
    thinking: "",
    deferred: true,
  });
  // 最后一条 assistant 保留内联思考内容，避免输出完毕后重复加载
  assert.equal(deferred.messages[3].content[0].thinking, "latest reasoning");
  assert.equal(deferred.messages[3].content[0].deferred, undefined);

  const full = buildSessionContext(entries);
  assert.equal(full.messages[1].content[0].thinking, "old reasoning");
  assert.equal(full.messages[3].content[0].thinking, "latest reasoning");
});
