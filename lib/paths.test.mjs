...
test("filePathFromApiSegments restores UNC roots", async () => {
  const { filePathFromApiSegments } = await loadSubject();
  // Clients encode the UNC root inside the first segment because a literal
  // "//" is normalized away by URL routing.
  assert.equal(
    filePathFromApiSegments(["//192.0.2.1", "share", "dir"]),
    "//192.0.2.1/share/dir",
  );
  // Bare segments remain root-relative paths on every platform.
  assert.equal(
    filePathFromApiSegments(["192.0.2.1", "share", "dir"]),
    "/192.0.2.1/share/dir",
  );
